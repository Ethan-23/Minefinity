...
public class CommandFeedback {
    ...
    public static Component GET_PLAYER_BALANCE(Player player, double balance){
        return TextConversions.parse("<yellow>" + player.getName() + "'s balance: <green>$" + balance);
    }

    public static Component ADD_PLAYERS_BALANCE(Player player, int amount){
        return TextConversions.parse("<green>$" + amount + " <yellow>has been added to " + player.getName() + "'s account");
    }

    public static Component SUB_PLAYERS_BALANCE(Player player, int amount){
        return TextConversions.parse("<green>$" + amount + " <yellow>has been removed from " + player.getName() + "'s account");
    }

    public static Component SET_PLAYERS_BALANCE(Player player, int amount){
        return TextConversions.parse("<yellow>" + player.getName() + "'s account has been set to <green>$" + amount);
    }
