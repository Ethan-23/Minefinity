...
public class StaffMode implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String @NotNull [] args) {
        ...
        if(!staffInventory.containsKey(player.getUniqueId())){
            ...
            player.getInventory().setItem(8, new ItemBuilder(Material.BLAZE_ROD, TextConversions.parse("<gold>Npc Stick")).build());
            player.getInventory().setItem(0, new ItemBuilder(Material.BREEZE_ROD, TextConversions.parse("<blue>Freeze Stick")).build());
        }else {
