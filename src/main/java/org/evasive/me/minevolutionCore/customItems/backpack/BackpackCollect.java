...
public class BackpackCollect {
    ...
    public String findBackpackItem(Player player, String itemId){
        ...
        for(ItemStack item : inventory.getContents()){
            ...
            if(storageAmount <= MinevolutionCore.playerManager.getBackpackStoredItemAmount(player, itemId)) continue;
    ...
    public void insertInventoryToBackpack(Player player, String backPackId){
        ...
        for(ItemStack item : inventory.getContents()){
            ...
            int currentStorage = MinevolutionCore.playerManager.getBackpackStoredItemAmount(player, itemId);
            int remainingStorage = maxItemStorage - currentStorage;

            if(remainingStorage == 0)continue;

            if(amount <= remainingStorage){
                MinevolutionCore.playerManager.addBackpackItem(player, itemId, amount);
                item.setAmount(0);
            }else {
                MinevolutionCore.playerManager.addBackpackItem(player, itemId, remainingStorage);
    ...
    public boolean canHoldItem(Player player, String backpackId, String itemId){
        ...
        int total = MinevolutionCore.playerManager.getBackpackStoredItemAmount(player, itemId);
        return total <= max;
    }
